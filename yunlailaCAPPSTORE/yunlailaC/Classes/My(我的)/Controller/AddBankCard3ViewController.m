...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section==0)
    {
        ...
        titleLable1.text = @"温馨提示：银行卡必须为认证姓名所属银行卡。";
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==1)
    {
        ...
        NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                         @"hex_client_waybill_openLoanServiceFunction",@"funcId",
...
//             申请开通代收款业务：如果返回：-20012,你们提示：该银行卡已经被绑定过了
             if (obj.global.flag.intValue!=1)
             {
                 [MBProgressHUD showAutoMessage:@"服务器异常，请稍后再试"];
                 return;
             ...
             if (((response *)obj.responses[0]).flag.intValue==1)
             {
                 [MBProgressHUD showAutoMessage:@"开通代收款审核信息提交成功，请耐心等待工作人员处理"];
                 [self dismissViewControllerAnimated:YES completion:nil];
             ...
             {
                 [MBProgressHUD showAutoMessage:@"该银行卡已经被绑定过了"];
                 return;
             }
             else
             {
         ...
         } Failure:^(NSError *error)
         {
             NSLog(@"%@",error);
             [MBProgressHUD hideHUDForView:self.view animated:YES];
             [MBProgressHUD showError:@"开通代收款审核信息失败" toView:self.view];
         }];
