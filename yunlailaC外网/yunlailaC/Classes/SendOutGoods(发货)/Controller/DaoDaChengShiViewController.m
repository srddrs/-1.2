...
#import "DaoDaDiQuViewController.h"
@interface DaoDaChengShiViewController ()<UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout,UITextFieldDelegate>
{
...
@implementation DaoDaChengShiViewController
@synthesize sheng;
- (void)setSheng:(NSDictionary *)sheng1
{
    ...
    NSArray *shi = [sheng1 objectForKey:@"child"];
    [items addObjectsFromArray:shi];
...
-(void)buttonClick:(UIButton *)sender
{
    ...
    [_collectionView reloadData];
    NSDictionary *shi = [items objectAtIndex:sender.tag];
    
    
    NSArray *diqus = [shi objectForKey:@"child"];
    if (diqus.count>0)
    {
        DaoDaDiQuViewController *vc =[[DaoDaDiQuViewController alloc] init];
    ...
    {
        NSString *text = [NSString stringWithFormat:@"%@%@",[sheng objectForKey:@"receipt_name"],[shi objectForKey:@"receipt_name"]];
//        NSString *text = [NSString stringWithFormat:@"%@",[shi objectForKey:@"receipt_name"]];
        NSDictionary *info = @{
                               @"text" :text ,
                               @"receipt_region_id" : [shi objectForKey:@"receipt_region_id"],
                               };
