//
//  ChangYongSendAddressViewController.m
//  yunlailaC
//
...
@interface ChangYongSendAddressViewController ()<UITableViewDataSource,UITableViewDelegate,SWTableViewCellDelegate>
{
...
@implementation ChangYongSendAddressViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
    {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(addAndSetFaHuo:)
                                                     name:KNOTIFICATION_addAndSetFaHuo
                                                   object:nil];
    }
    return self;
}

-(void)addAndSetFaHuo:(NSNotification *)notification
{
    if (isSetting==NO)
    {
        ...
        NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                         @"hex_client_shipaddress_list",@"funcId",
                                         [NSString stringWithFormat:@"%d",0],@"start",
                                         [NSString stringWithFormat:@"%d",10],@"limit",
        ...
        [helper Soap:commonURL Parameters:paramDic Success:^(id responseObject)
         {
             ...
             if (((response *)obj.responses[0]).flag.intValue==1)
             {
                 ...
                 if (items.count>0)
                 {
                     NSDictionary *addressInfo = [items objectAtIndex:0];
                     
                     [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_XuanChangyong object:nil userInfo:addressInfo];
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    UILabel *usernameLable = [[UILabel alloc] initWithFrame:CGRectMake(15*APP_DELEGATE().autoSizeScaleX, 5, 160*APP_DELEGATE().autoSizeScaleX, 35)];
...
//    usernameLable.frame = CGRectMake(15*APP_DELEGATE().autoSizeScaleX, 0, size.width*APP_DELEGATE().autoSizeScaleX,35);
    ...
    addressLable.text = [NSString stringWithFormat:@"%@%@",[item objectForKey:@"region_city"],[item objectForKey:@"address"]];
...
- (void)setDefBtnClick:(UIButton *)sender
{
    ...
    [MBProgressHUD showMessag:@"" toView:self.view];
    NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                     @"hex_client_shipaddress_default",@"funcId",
                                     [addressInfo objectForKey:@"ship_address_id"],@"shipAddressId",
                                     nil];
...
{
    NSDictionary *addressInfo = [items objectAtIndex:sender.tag];
    EditSendAddressViewController *vc =[[EditSendAddressViewController alloc] init];
...
- (void)swipeableTableViewCell:(SWTableViewCell *)cell didTriggerRightUtilityButtonWithIndex:(NSInteger)index {
    switch (index) {
        case 0:
        {
            ...
            [UIAlertView showAlert:@"是否确认删除该发货地址？" delegate:self cancelButton:@"取消" otherButton:@"删除" tag:indexPath.section];
        }
            break;
        default:
            break;
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (buttonIndex==1)
    {
        ...
        [MBProgressHUD showMessag:@"" toView:self.view];
        NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                         @"hex_client_shipaddress_delete",@"funcId",
                                         [NSString stringWithFormat:@"%@",[addressInfo objectForKey:@"ship_address_id"]],@"ship_address_id",
        ...
        [helper Soap:commonURL Parameters:paramDic Success:^(id responseObject)
         {
             ...
             if (((response *)obj.responses[0]).flag.intValue==1)
             {
                 [MBProgressHUD showAutoMessage:@"删除发货地址成功"];
                 [self loadData];
             }
             else
             {
         ...
         } Failure:^(NSError *error)
         {
             NSLog(@"%@",error);
             [MBProgressHUD hideHUDForView:self.view animated:YES];
             [MBProgressHUD showError:@"删除发货地址失败" toView:self.view];
         }];
