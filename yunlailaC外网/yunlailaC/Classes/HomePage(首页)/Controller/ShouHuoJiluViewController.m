...
- (void) initView
{
    ...
    [segmentMenu segmentWithTitles:@[@"未收货运单",@"已收货运单"]];

    [self.view addSubview:segmentMenu];
    
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 31*APP_DELEGATE().autoSizeScaleY, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - TabbarHeight - statusViewHeight-31*APP_DELEGATE().autoSizeScaleY)];
...
- (void)requestServer
{
   ...
   NSDictionary *loginUser = [[NSUserDefaults standardUserDefaults] objectForKey:kUserInfo];
    ...
    if (type==0)
    {
        [paramDic setObject:@"2" forKey:@"is_receive"];
    }
    else
    {
        [paramDic setObject:@"1" forKey:@"is_receive"];
    }
