//
//  EditGetAddressViewController.m
//  yunlailaC
//
...
@interface EditGetAddressViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,DingWeiDiZhiGetViewControllerDelegate,AMapSearchDelegate>
{
...
@implementation EditGetAddressViewController
@synthesize addInfo;
- (void) setAddInfo:(NSDictionary *)addInfo1
{
    ...
    kaitongquyuLabel.text = [NSString stringWithFormat:@"%@",[addInfo objectForKey:@"recript_city"]];
    xiangxidizhiText.text = [NSString stringWithFormat:@"%@",[addInfo objectForKey:@"address"]];
    receipt_region_id = [NSString stringWithFormat:@"%@",[addInfo objectForKey:@"receipt_region_id"]];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==1)
    {
        ...
        NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                         @"hex_client_receiptaddress_update",@"funcId",
                                         [addInfo objectForKey:@"recript_address_id"],@"recript_address_id",
        ...
        [helper Soap:commonURL Parameters:paramDic Success:^(id responseObject)
         {
             ...
             {
                 [MBProgressHUD showAutoMessage:@"修改收货地址成功"];
                 [self.navigationController popViewControllerAnimated:YES];
             }
             else
             {
         ...
         } Failure:^(NSError *error)
         {
             ...
             [MBProgressHUD showError:@"修改收货地址失败" toView:self.view];
         }];
