//
//  AddSendAddressViewController.m
//  yunlailaC
//
...
@interface AddSendAddressViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,DingWeiDiZhiViewControllerDelegate>
{
...
@implementation AddSendAddressViewController
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    else   if(indexPath.section==0&&indexPath.row==1)
    {
        ...
        if (!lianxidianhuaText)
        {
            ...
            if ([[loginUser objectForKey:@"account_id"] length]>0) {
                lianxidianhuaText.text = [NSString stringWithFormat:@"%@",[loginUser objectForKey:@"account_id"]];
            }
    ...
    else   if(indexPath.section==0&&indexPath.row==2)
    {
        ...
        if (!kaitongquyuLabel)
        {
            ...
            kaitongquyuLabel.textColor = [UIColor colorWithRed:198/255.0 green:198/255.0 blue:204/255.0 alpha:1];
    ...
    else   if(indexPath.section==1&&indexPath.row==0)
    {
        ...
        if (!gouBtn)
        {
            ...
            [gouBtn setImage:[UIImage imageNamed:@"moren_xuanzhong@3x"]  forState:UIControlStateSelected];            [gouBtn addTarget:self
    ...
    if (alertView.tag==2)
    {
        if (buttonIndex==1)
        {
            ...
            NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                             @"hex_client_shipaddress_save_function",@"funcId",
            ...
            [helper Soap:commonURL Parameters:paramDic Success:^(id responseObject)
             {
                 ...
                 {
                     [MBProgressHUD showAutoMessage:@"新增发货地址成功"];
                     [self.navigationController dismissViewControllerAnimated:YES completion:^{
                         //选第一个
                         [[NSNotificationCenter defaultCenter] postNotificationName:KNOTIFICATION_addAndSetFaHuo object:nil userInfo:nil];
                     }];
                 }
                 else
                 {
             ...
             } Failure:^(NSError *error)
             {
                 NSLog(@"%@",error);
                 [MBProgressHUD hideHUDForView:self.view animated:YES];
                 [MBProgressHUD showError:@"新增发货地址失败" toView:self.view];
             }];
