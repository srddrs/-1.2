...
@interface AddBankCard4ViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
@implementation AddBankCard4ViewController
...
-(void)getBankInfo:(NSString *)bankId
{
    ...
    NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                     @"hex_client_bank_queryOtherBankInfoByIdFunction",@"funcId",
                                     bankId,@"cust_bank_id",
                                     nil];
    ...
    [helper Soap:commonURL Parameters:paramDic Success:^(id responseObject)
     {
         ...
         if (((response *)obj.responses[0]).flag.intValue==1)
         {
             ...
             NSLog(@"item:%@",item);
             [bankDict removeAllObjects];
             [bankDict setDictionary:item];
...
- (void) initView
{
    _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height-TabbarHeight)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    if (section==0)
    {
        ...
        titleLable1.text = @"温馨提示：他人银行卡认证后一经认证无法修改。";
        [headView addSubview:titleLable1];
        
        UILabel *titleLable2 = [[UILabel alloc] initWithFrame:CGRectMake(10*APP_DELEGATE().autoSizeScaleX + 20, 25, 290*APP_DELEGATE().autoSizeScaleX, 25)];
        ...
        titleLable2.text = [NSString stringWithFormat:@"拒绝理由:%@",[bankDict objectForKey:@"audit_note"]];
//        audit_note
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath
{
    ...
    if(indexPath.section==1&&indexPath.row==0)
    {
        ...
        [photoView1 sd_setImageWithURL:[NSURL URLWithString:[bankDict objectForKey:@"card_positive_img"]] placeholderImage:[UIImage imageNamed:@"smrz_tianjia"]];
        photoViewURL1 = [bankDict objectForKey:@"card_positive_img"];
        ...
        [photoView2 sd_setImageWithURL:[NSURL URLWithString:[bankDict objectForKey:@"card_back_img"]] placeholderImage:[UIImage imageNamed:@"smrz_tianjia"]];
        photoViewURL2 = [bankDict objectForKey:@"card_back_img"];
        ...
        photoViewURL3 = [bankDict objectForKey:@"other_img"];
        [photoView3 sd_setImageWithURL:[NSURL URLWithString:[bankDict objectForKey:@"other_img"]] placeholderImage:[UIImage imageNamed:@"smrz_tianjia"]];
    ...
    if(indexPath.section==2&&indexPath.row==0)
    {
        ...
        xingmingText.text = [NSString stringWithFormat:@"%@",[bankDict objectForKey:@"bank_username"]];
//        xingmingText.enabled = NO;
    ...
    else if(indexPath.section==2&&indexPath.row==1)
    {
        ...
        shenfenzhengText.text = [NSString stringWithFormat:@"%@",[bankDict objectForKey:@"id_card"]];
//        shenfenzhengText.enabled = NO;
    ...
    else if(indexPath.section==2&&indexPath.row==2)
    {
        ...
        if (!yinhangkaText)
        {
            ...
            yinhangkaText.frame = CGRectMake(75*APP_DELEGATE().autoSizeScaleX, 0, 228*APP_DELEGATE().autoSizeScaleX, 38*APP_DELEGATE().autoSizeScaleY);
            yinhangkaText.delegate = self;
            yinhangkaText.placeholder = @"请输入银行卡号";
            yinhangkaText.returnKeyType = UIReturnKeyDone;
            yinhangkaText.keyboardType = UIKeyboardTypePhonePad;
            yinhangkaText.font = viewFont2;
            yinhangkaText.textColor = fontColor;
            yinhangkaText.text = @"";
            
        }
        yinhangkaText.text = [NSString stringWithFormat:@"%@",[bankDict objectForKey:@"bankcard_no"]];
        yinhangkaText.enabled = NO;
        [shenfenzhengView addSubview:yinhangkaText];
    ...
    else if(indexPath.section==2&&indexPath.row==3)
    {
        ...
        UIButton *submitBtn = [[UIButton alloc] initWithFrame: CGRectMake(10*APP_DELEGATE().autoSizeScaleX, 38*APP_DELEGATE().autoSizeScaleY, 300*APP_DELEGATE().autoSizeScaleX, 34*APP_DELEGATE().autoSizeScaleY)];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (buttonIndex==1)
    {
        ...
        NSMutableDictionary *paramDic = [[NSMutableDictionary alloc] initWithObjectsAndKeys:
                                         @"hex_client_bank_againBindOtherBankFunction",@"funcId",
                                         [bankDict objectForKey:@"cust_bank_id"],@"cust_bank_id",
                                         ...
                                         yinhangkaText.text,@"bankcard_no",
                                         nil];
        ...
        [helper Soap:commonURL Parameters:paramDic Success:^(id responseObject)
         {
             ...
             if (((response *)obj.responses[0]).flag.intValue==1)
             {
                 [MBProgressHUD showAutoMessage:@"提交银行卡(他人)信息成功，请耐心等待工作人员处理"];
                 [self dismissViewControllerAnimated:YES completion:nil];
