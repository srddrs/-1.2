...
- (void)initView
{
    ...
    //    cycleScrollView.placeholderImage = [UIImage imageNamed:@"zhuye_banner"];
    //网络图片
    //    NSMutableArray *images = [[NSMutableArray alloc] init];
    //    for (int i=0; i<anlis.count; i++)
    //    {
    //        NSDictionary *anli = [anlis objectAtIndex:i];
    //        [images addObject:[anli objectForKey:@"titlePage"]];
    //    }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    else
    {
        YLLWebViewController *vc = [[YLLWebViewController alloc] init];
        vc.webTitle = @"运单跟踪";
        vc.webURL = [NSString stringWithFormat:@"%@?waybillNum=%@&type=1",waybillDetailsURL,textField.text];
        YLLNavigationController *nav = [[YLLNavigationController alloc] initWithRootViewController:vc];
        nav.modalTransitionStyle = UIModalTransitionStyleFlipHorizontal;
        [self presentViewController:nav animated:YES completion:nil];
